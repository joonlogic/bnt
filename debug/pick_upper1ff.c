...
//	fseek( fp, sizeof(bheader)*id, SEEK_SET ); //skip to Year 2017
	...
	do {
		...
		if((picknonce & 0xFF800000) != 0xFF800000) continue;
	...
	char buf[64] = {0,};
	//Lowest
	sprintf(buf, "BH_LOWNONCE_FF8[0]_%d.bin", lowcount);
	...
	//2nd Lowest
	sprintf(buf, "BH_LOWNONCE_FF8[1]_%d.bin", lowcount2nd);
